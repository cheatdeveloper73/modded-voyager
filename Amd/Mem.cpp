...
auto mm::translate(host_virt_t host_virt) -> u64
{
	...
	if (reinterpret_cast<ppde>(cursor.value)[virt_addr.pd_index].large_page)
		return (reinterpret_cast<ppde>(cursor.value)[virt_addr.pd_index].pfn << 21) + virt_addr.offset_2mb;
...
auto mm::translate_guest_physical(guest_phys_t phys_addr, map_type_t map_type) -> u64
{
	...
	const auto npt_pml4 = reinterpret_cast<pnpt_pml4e>(map_page(vmcb->ncr3, map_type));
	if (!npt_pml4[guest_phys.pml4_index].present)
		return 0;

	const auto npt_pdpt = reinterpret_cast<pnpt_pdpte>(map_page(npt_pml4[guest_phys.pml4_index].pfn << 12, map_type));
	if (!npt_pdpt[guest_phys.pdpt_index].present)
		return 0;

	const auto npt_pd = reinterpret_cast<pnpt_pde>(map_page(npt_pdpt[guest_phys.pdpt_index].pfn << 12, map_type));
	if (!npt_pd[guest_phys.pd_index].present)
		return 0;

	if (reinterpret_cast<pnpt_pde_2mb>(npt_pd)[guest_phys.pd_index].large_page)
		return (reinterpret_cast<pnpt_pde_2mb>(npt_pd)[guest_phys.pd_index].pfn << 21) + guest_phys.offset_2mb;

	const auto npt_pt = reinterpret_cast<pnpt_pte>(map_page(npt_pd[guest_phys.pd_index].pfn << 12, map_type));
...
auto mm::read_guest_phys(guest_phys_t dirbase, guest_phys_t guest_phys, guest_virt_t guest_virt, u64 size) -> svm::vmxroot_error_t
{
	while (size)
	{
...
auto mm::write_guest_phys(guest_phys_t dirbase, guest_phys_t guest_phys, guest_virt_t guest_virt, u64 size) -> svm::vmxroot_error_t
{
	while (size)
	{
		auto dest_current_size = PAGE_4KB - virt_addr_t{ guest_virt }.offset_4kb;
...
auto mm::copy_guest_virt(guest_phys_t dirbase_src, guest_virt_t virt_src, guest_virt_t dirbase_dest, guest_virt_t virt_dest, u64 size) -> svm::vmxroot_error_t
{
	while (size)
	{
